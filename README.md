# Deki Audio

Documentation: https://dekiengine.github.io/deki-audio/ (components and properties, generated from the code)

Audio playback peripheral interface for the Deki Engine, with a MAX98357 (I²S) backend.

Part of the [Deki Engine](https://github.com/dekiengine/deki-engine) package ecosystem.

## Namespace

This package's types live in `DekiAudio`. Scene files store the qualified
name, so a component is `DekiAudio::SomeComponent` there, and code naming one
needs the namespace:

```cpp
using namespace DekiAudio;
obj->AddComponent<SomeComponent>();
```

Scenes saved before 0.16.0 used bare names and still load: every component
records what it used to be called, and a save writes the current name.

## Installation

Install via the Package Manager inside the Deki Editor.

## License

Licensed under the Apache License, Version 2.0. See [LICENSE](LICENSE) for details.
